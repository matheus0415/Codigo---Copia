...
class Cliente:
        ...
        while True:
            ...
            if msg.strip() != "":  # Verifica se a mensagem não está vazia
                ...
                if msg.lower().startswith("detentor"):
                    chave = msg.split()[1]  # Supondo que a chave esteja na segunda posição
                ...
                self.recieve()  # Recebe a resposta do servidor
    ...
    def send_protocol(self, k, ip, porta, comando):
        ...
        self.recieve()  # Recebe a resposta do sucessor
    ...
    def enviar_com_finger_table(self, k, ip, porta, comando):
        ...
        for finger in reversed(self.info.finger_table):
            if finger <= k:
                self.sc.connect((self.info.HOST_SERVER, finger))
                break
