...
class ComunicadorTCPHandler(socketserver.BaseRequestHandler):
    ...
    def determinar_no(self, chave):
        ...
        # Esta função deve ser implementada de acordo com a lógica específica do seu sistema
        # Exemplo de lógica simples: retorna um nó baseado no valor da chave
        try:
            if chave == -1:
                return "NÓ_INVÁLIDO"
            no = f"NÓ_{chave % 30}"  # Exemplo de lógica: retorna um nó baseado no valor da chave
            return no
        except ValueError:
